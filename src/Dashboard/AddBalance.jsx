...
function AddBalance() {
  ...
  const handleUpdateData = async () => {
    try {
      let url = "";
      if (method === "1") {
        url = `http://localhost:8080/api/v1/dompetku/wallet/deposit/${getWalletId}?amount=${amount}`;
      } else if (method === "2") {
        url = `http://localhost:8080/api/v1/dompetku/wallet/decrease/${getWalletId}?amount=${amount}`;
      } else {
    ...
    <>
      <div className="popup show">
        <div className="form-update">
          ...
          <button className="btn-icon2 font-12" onClick={handleUpdateData}>
            Update Data
          </button>
