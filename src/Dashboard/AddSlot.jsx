...
function AddSlot() {
  ...
  const handleAddSlot = async () => {
    try {
      const response = await fetch(`http://localhost:8080/api/v1/dompetku/wallet/addNewSlot/${ownerId}?name=${name}&balance=${balance}`, {
        method: 'POST',
        headers: {
  ...
  return (
    ...
    <div id="" className="popup show">
      <div className="form-update">
        <div className="font-24 black-color" style={{ textAlign: 'center' }}>New Slots</div>
          ...
          <div className="label-input black-color hidden">
            ...
            <input type="text" name="balance" placeholder="Balance" value={balance} onChange={e => setBalance(e.target.value)}/>
          </div>
          <button className="btn-icon2 font-12" onClick={handleAddSlot} >Update</button>
